...
public class AutoPlayer extends Player{
    Random rand;
    public AutoPlayer(int tempstorelimit,int id){super(tempstorelimit,id);
    rand=new Random();}
    ...
    public void startplanning(ArrayList<Champion> listOfChampions){
        ...
        if(currentChampions.size()>0)//&& ChampionsNotStunned() && ChampionsNotDead()
            System.out.println("2-Champions' Interactions");
        if(currentChampions.size()>0)
        choice=rand.nextInt(3);
        else choice=rand.nextInt(2);
        switch(choice) {
            case 0:
                return ;
            case 1:
    ...
    public void buyNewChamps(ArrayList<Champion> listOfChampions) {
        ...
        for (i = 0; i < tempstorelimit; i++) {
            choice = rand.nextInt(tempstorelimit);
            if (choice == 0)
                break;
            ...
            x = rand.nextInt(Arena.getArena().getX());
            System.out.print("y:");
            y = rand.nextInt(Arena.getArena().getY());
    ...
    public void interactWithChampions(){
        championsTargets();
        int choice1=-1;
        int choice2=-1;
        ...
        while(true) {
            ...
            choice1 = rand.nextInt(currentChampions.size());
            if (choice1 == 0)
                break;
